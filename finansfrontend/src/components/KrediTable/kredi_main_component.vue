...
<template>
  <main id="main_11" class="h-fit">
    <div
        id="kisi"
        class="font-bold h-100vh pl-4 my-1 rounded-2xl py-2 fixedElement"
    >
      <h1 class="text-xl">Krediler</h1>
      <a class="text-xl transition-all">{{ kredi_store.sayfa + 1 }}. sayfada {{ kredi_store.krediler.length }} Tane kayıt
    ...
    <table class="h-[2px] ">
      <tr>
        <th v-if="kredi_store.id_order==='?sırala=ar_id'" class="cursor-pointer" @click="kredi_store.order_by_id()"><a>ID</a> <a
            class="font-bold text-green-500">(asc)</a></th>
        <th v-if="kredi_store.id_order==='?sırala=az_id'" class="cursor-pointer" @click="kredi_store.order_by_id()"><a>ID</a> <a
        ...
        <th>
          <a class="-ml-8">logo</a>
          <button @click="kredi_store.yukle(kredi_store.sayfa=0);kredi_store.get_all_kredi()"
    ...
    <button @click="kredi_store.sonraki_sayfa()" v-bind:class="{ 'bg-gray-600': kredi_store.at_end }" class="btn bg:var(--menu_arkaplan)">
      Sonraki
    </button>
...
.loader {
  ...
  border-top: 5px solid #161612; /* Blue */
  border-radius: 50%;

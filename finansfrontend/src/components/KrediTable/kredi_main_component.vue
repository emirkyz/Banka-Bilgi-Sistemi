...
<template>
  <main id="main_11" class="h-fit">
    ...
    <table class="h-[2px] table-fixed ">
      <thead>
      <tr>
        <th v-if="kredi_store.id_order==='?sırala=ar_id'" class="cursor-pointer" @click="kredi_store.order_by_id()"><a>ID</a>
          <a class="font-bold text-green-500">(asc)</a></th>
        <th v-if="kredi_store.id_order==='?sırala=az_id'" class="cursor-pointer" @click="kredi_store.order_by_id()"><a>ID</a>
